...
fn test_handle_task_failure_escalation_at_two_then_block_at_three() {
    ...
    handle_task_failure(&mut conn, "TASK-A", 1, &mut ctx, None, None, None, None).unwrap();
    ...
    handle_task_failure(&mut conn, "TASK-A", 2, &mut ctx, None, None, None, None).unwrap();
    let (count, model, status, _) = read_task_state(&conn, "TASK-A");
    ...
    handle_task_failure(&mut conn, "TASK-A", 3, &mut ctx, None, None, None, None).unwrap();
...
fn test_handle_task_failure_max_retries_one_blocks_immediately() {
    ...
    let mut ctx = IterationContext::new(8);
    handle_task_failure(&mut conn, "BLOCK-FAST", 1, &mut ctx, None, None, None, None).unwrap();
...
fn test_handle_task_failure_max_retries_two_blocks_on_second() {
    ...
    handle_task_failure(&mut conn, "BLOCK-TWO", 1, &mut ctx, None, None, None, None).unwrap();
    ...
    handle_task_failure(&mut conn, "BLOCK-TWO", 2, &mut ctx, None, None, None, None).unwrap();
    let (count, _, status, _) = read_task_state(&conn, "BLOCK-TWO");
    assert_eq!(count, 2, "failure 2: count must be 2");
    assert_eq!(
        status, "blocked",
...
fn test_model_escalation_haiku_to_sonnet_at_two_failures() {
    ...
    let mut ctx = IterationContext::new(8);
    let result =
        escalate_task_model_if_needed(&conn, "HAIKU-T", 2, &mut ctx, None, None, None, None)
            .unwrap();
    assert_eq!(
        result,
...
fn test_model_escalation_empty_and_whitespace_normalize_to_opus() {
    for (idx, bad) in ["", "   ", "\t"].iter().enumerate() {
        ...
        let mut ctx = IterationContext::new(8);
        let result =
            escalate_task_model_if_needed(&conn, &id, 2, &mut ctx, None, None, None, None).unwrap();
        assert_eq!(
            result,
...
fn test_handle_task_failure_haiku_escalates_to_sonnet_at_two() {
    ...
    handle_task_failure(&mut conn, "HAIKU-PIPE", 1, &mut ctx, None, None, None, None).unwrap();
    ...
    handle_task_failure(&mut conn, "HAIKU-PIPE", 2, &mut ctx, None, None, None, None).unwrap();
