...
// FR-002 hard break: `task-mgr init` no longer auto-migrates legacy model
// routing keys (the `update_project_config_format` rewrite was deleted). Legacy
// keys now hard-error at the loop/batch preflight; the former
// `test_init_project_updates_legacy_model_routing_config` test was removed with
// the migration machinery. Re-coverage of the new `models init
// --force-replace-legacy` path is TEST-010 / FEAT-009.
