...
mod tests {
    ...
    // --- PERM-FEAT-005: Named permission mode tests ---

    /// Unit test: Dangerous mode → --dangerously-skip-permissions, NOT --permission-mode.
    #[test]
    fn test_permission_mode_dangerous() {
        let result = spawn_claude_echo("prompt", None, None, false, &PermissionMode::Dangerous);
        let output = result.expect("echo should succeed").output;
        let output = output.trim();
    ...
    /// Unit test: Scoped mode with tools → --permission-mode dontAsk --allowedTools <tools>,
    /// NOT --dangerously-skip-permissions.
    #[test]
    fn test_permission_mode_scoped_with_tools() {
        let tools = "Read,Edit,Write";
        let mode = PermissionMode::Scoped {
            allowed_tools: Some(tools.to_string()),
        };
        ...
        assert!(
            output.contains("--permission-mode dontAsk"),
            "Scoped+tools must include '--permission-mode dontAsk', got: '{output}'"
        );
        assert!(
            output.contains("--allowedTools"),
            "Scoped+tools must include --allowedTools, got: '{output}'"
        );
        assert!(
            output.contains(tools),
            "Scoped+tools must include the tools string, got: '{output}'"
        );
        assert!(
            !output.contains("--dangerously-skip-permissions"),
            "Scoped mode must NOT include --dangerously-skip-permissions, got: '{output}'"
    ...
    /// Unit test: Scoped mode with no tools → --permission-mode dontAsk,
    /// NOT --allowedTools, NOT --dangerously-skip-permissions.
    #[test]
    fn test_permission_mode_scoped_no_tools() {
        let mode = PermissionMode::Scoped {
            allowed_tools: None,
        };
        let result = spawn_claude_echo("prompt", None, None, false, &mode);
        ...
        assert!(
            output.contains("--permission-mode dontAsk"),
            "Scoped no-tools must include '--permission-mode dontAsk', got: '{output}'"
        );
        assert!(
            !output.contains("--allowedTools"),
            "Scoped no-tools must NOT include --allowedTools, got: '{output}'"
        );
        assert!(
            !output.contains("--dangerously-skip-permissions"),
            "Scoped no-tools must NOT include --dangerously-skip-permissions, got: '{output}'"
        );
    }

    /// Unit test: Auto mode → --enable-auto-mode, NOT --dangerously-skip-permissions,
    /// NOT --permission-mode.
    #[test]
    fn test_permission_mode_auto() {
        let result = spawn_claude_echo("prompt", None, None, false, &PermissionMode::Auto);
        let output = result.expect("echo should succeed").output;
        let output = output.trim();
