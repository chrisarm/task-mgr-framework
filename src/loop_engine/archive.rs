...
/// Outcome of processing a single PRD during archive.
enum PrdArchiveOutcome {
    Archived {
...
///
/// Returns `Archived` with the summary and moved items if the PRD is complete,
/// or `Skipped` with the reason if it should not be archived.
fn archive_single_prd(
...
mod tests {
    ...
    fn test_three_prds_complete_incomplete_no_prefix() {
        ...
        // PB: incomplete
        insert_prd(&conn, 2, "project-b", "feat/branch-b", Some("PB"));
        insert_task(&conn, "PB-001", "PB Task", 1, "in_progress");
        insert_prd_file(&conn, 2, "project-b.json", "task_list");

        // PC: NULL task_prefix
        insert_prd(&conn, 3, "project-c", "feat/branch-c", None);
        insert_prd_file(&conn, 3, "project-c.json", "task_list");
        drop(conn);
    ...
    fn test_archive_folder_uses_stripped_branch_name() {
        ...
        insert_prd(&conn, 1, "project-a", "feat/cool-feature", Some("PA"));
