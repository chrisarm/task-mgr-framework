...
//! parallel-slot wave pipeline's hot path: eligible-group selection plumbing
//! (`build_slot_contexts`, `build_shared_slot_params`, `build_slot_prompt_params`),
//! the threaded claim-and-spawn fan-out (`run_parallel_wave`), the FEAT-002
//! reset/halt contract (`apply_merge_fail_reset_and_halt_check`), the post-merge
//! reconcile (`apply_post_merge_reconcile`), and the wave entry point
//! (`run_wave_iteration`). The non-hot-path wave-decision helpers — the per-wave
//! preflight (`wave_preflight_check`) and the empty-group handlers
//! (`handle_no_eligible_tasks`, `handle_ephemeral_deadlock`) — were relocated to
//! the `wave_orchestration` submodule (REFACTOR-003, a behavior-neutral move);
//! `run_wave_iteration` calls them across that module boundary.
//!
...
use crate::loop_engine::wave_orchestration::{
    handle_ephemeral_deadlock, handle_no_eligible_tasks, wave_preflight_check,
};
use crate::output::ui;
