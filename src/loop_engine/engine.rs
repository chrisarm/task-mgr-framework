...
// integration tests rely on stays valid (FR-008). The remaining three are
// engine-internal: re-exported `pub(super)` so the wave call sites
// (`run_parallel_wave`, `run_wave_iteration`) and the inline test modules keep
// calling them by their bare names without a per-site module qualifier.
#[allow(deprecated)]
pub(super) use crate::loop_engine::slot::claim_slot_task;
pub use crate::loop_engine::slot::run_slot_iteration;
pub(super) use crate::loop_engine::slot::{process_slot_result, slot_failure_result};
