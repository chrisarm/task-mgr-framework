//! Migration 19: Add slot column to run_tasks for parallel execution tracking.
//!
//! ## Changes
...
/// Migration 19: Add slot column to run_tasks.
...
mod tests {
    ...
    /// AC: schema_version is >= 19 after running all migrations.
    #[test]
    fn test_v19_schema_version() {
        let (_temp_dir, conn) = setup_migrated_db();

        let version = get_schema_version(&conn).unwrap();
        assert!(
            version >= 19,
            "schema_version must be >= 19 after running migrations, got {version}"
        );
    }

    /// AC: slot column exists on run_tasks with correct default.
    #[test]
    fn test_v19_slot_column_exists_with_default() {
        let (_temp_dir, conn) = setup_migrated_db();
        ...
        assert!(
            exists,
            "run_tasks.slot column must exist after v19 migration"
        );
    ...
    #[test]
    fn test_v19_slot_column_writable() {
        let (_temp_dir, conn) = setup_migrated_db();
    ...
    /// AC: down migration reverts schema_version to 18 (v18 dedup_dismissals remains applied).
    #[test]
    fn test_v19_migration_down() {
        let temp_dir = TempDir::new().unwrap();
        let mut conn = open_connection(temp_dir.path()).unwrap();
        create_schema(&conn).unwrap();
        run_migrations(&mut conn).unwrap();
        ...
        assert!(
            MIGRATIONS.iter().any(|m| m.version == 19),
            "v19 must be registered in MIGRATIONS"
        );
