...
**Low-value areas**: If the source material (PRD or prior spike) explicitly marked capabilities as low-value/high-effort in a “5.5” section, treat them as out of scope unless the user re-opens the question.
...
**CONTRACT-xxx detection**: Before generating the task list, check `task-mgr list --prefix $PREFIX --task-type contract` (or simply look at what `task-mgr next` would return). If a `CONTRACT-xxx` already exists for this PRD, make the dependent FEAT tasks depend on it.
...
- Explicit per-task `model` is rung 1 and bypasses ALL config routing — never emit it; it exists only for human one-offs.
- `routing.byIdPrefix` / `routing.taskClasses` (rungs 2–3) — operator-configured forced routes (`task-mgr models route <prefix>`). Review-class IDs (`REVIEW-*`, `CODE-REVIEW-*`, `MILESTONE-FINAL`) carry a built-in, non-redefinable force to the **frontier tier**.
- Anchor window (rung 5) — the default path: `models.anchor` (default `standard`) + difficulty offset picks the capability tier (`low` → anchor−1, `medium` → anchor, `high` → anchor+1, clamped to the ladder), then the provider's tier ladder maps tier → model.
...
**Before using this skill on anything with high abstraction risk**: consider running `/spike` first. The spike may emit a `CONTRACT-xxx` predecessor (for foundations that will be used by 2+ later tasks). You can then include that CONTRACT task in the lean list you generate.
