...
- `doctor-path-twins` reports `path_identity_twin` when two `prd_metadata` rows share pin-19 path identity; auto-fix drops empty-side metadata only.
...
- **Path-identity twins (`doctor-path-twins`).** Fresh import never seeds twins (init refuses 2+). Seeding relative+absolute `prd_files` twins requires raw SQL `INSERT`, which this harness forbids — record as `verified-unreachable` with precondition “verify-task-mgr forbids writing SQLite yourself; twin collapse is proven by `commands::doctor` unit tests (`test_path_identity_twin_empty_side_autofix_collapses`, `test_path_identity_twin_both_live_report_only`).” Still confirm healthy doctor JSON includes `path_identity_twins` in `summary` (value `0`) after import.
...
- `doctor --auto-fix` mutates tasks (stale `in_progress` → `todo`, abandoned runs) and may `DELETE` empty-side `prd_metadata` for `path_identity_twin` (never archives, never moves JSON). Prefer JSON `dry_run` / `--dry-run` before repair.
- `path_identity_twin` auto-fix requires zero unarchived tasks on one side (`archived_at IS NULL` count, including done/irrelevant). Both-live twins are report-only.
