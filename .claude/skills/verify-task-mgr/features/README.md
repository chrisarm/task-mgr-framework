...
- [Loop stop](./loop-stop.md) covers nested `loop stop --prefix` help, missing-record refusal, invalid-prefix refusal, and the skipped live-loop path (harness refuses `loop run` / `batch run`).
